...
def select_front_leg_fallback(mask: np.ndarray,
    ...
    # Band half-width: must cover the widest part of the leg (hoof / fetlock)
    # which is significantly wider than the cannon.  60 % of the seed bounding
    # box half-width is a safe lower bound; the minimum of cannon_w+40 and
    # 12 % of image width are kept as hard floors.
    cannon_w = _cannon_width(seed['mask'])
    band_half = max(int(bw_s * 0.60), cannon_w + 40, int(w * 0.12))
...
def select_front_leg_from_keypoints(mask: np.ndarray,
    ...
    band_half = max(int(bw_c * 0.60), cannon_w + 40, int(w * 0.12))
    col_l = max(0, cx_s - band_half)
    col_r = min(w, cx_s + band_half)
...
def find_cannon_bone_axis(leg_mask: np.ndarray,
    ...
    shaft_bot = top_y + int(leg_h * 0.85)   # was 0.75 — extend to include more cannon rows
    ...
    if inferencer is not None:
        ...
        if legs:
            ...
            if best_leg is not None:
                knee, hoof = best_leg
                lm = select_front_leg_from_keypoints(mask, knee, hoof, depth_map=depth_map, debug=do_debug)
                if lm is not None:
